...
export class SeederService {
  ...
  async seed(): Promise<boolean> {
    ...
    if (already) {
      this.logger.warn(
        'Datos ya existen. Usa npm run seed:fresh para reiniciar.',
      );
      return false;
    }
    ...
    this.logger.log(
      `→ ${animals.filter((animal) => animal.estado === 'disponible').length} disponibles · ${
        animals.filter((animal) => animal.estado === 'adoptado').length
      } adoptados`,
    );
  ...
  async clearAndSeed(): Promise<boolean> {
    ...
    this.logger.log('Tablas vaciadas.');
    return this.seed();
  }
}
